...
describe('Blog app', function() {
  beforeEach(function() {
    ...
    cy.createUser({ username: "test_user", password: "sekret", name: "Mikael"})
  ...
  describe("When logged in", function() {
    ...
    it("blogs are ordered by number of likes", function() {
      cy.get('.blog').eq(0).find("button").click()
        .parents("#blog").should('contain', 'likes 10')
      cy.get('.blog').eq(1).find("button").click()
        .parents("#blog").should('contain', 'likes 5')
      cy.get('.blog').eq(2).find("button").click()
        .parents("#blog").should('contain', 'likes 0')
    ...
    it("a blog can be created", function() {
      ...
      cy.get("#blogList")
        .should("contain", "New test blog title Mikael")
    })
    ...
    it("own blog can be deleted", function() {
      cy.contains("other users").parent().as("otherBlog")
        .find("button").click()
      cy.get("@otherBlog").should("not.contain", "delete")

      cy.get("#blogList")
        .contains("default blog").parent().as("theBlog")
        .find("button").click()
